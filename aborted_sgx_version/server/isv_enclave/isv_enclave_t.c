...
static sgx_status_t SGX_CDECL sgx_enclave_encrypt(void* pms)
{
	...
	if (_in_out_data) {
		if (memcpy_s(_tmp_out_data, _len_out_data, _in_out_data, _len_out_data)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
SGX_EXTERNC const struct {
	...
	10,
	{
		...
		{(void*)(uintptr_t)sgx_enclave_generate_key, 0, 0},
		{(void*)(uintptr_t)sgx_enclave_encrypt, 0, 0},
		{(void*)(uintptr_t)sgx_enclave_decrypt, 0, 0},
